<?php
  require_once __DIR__ . '/../../config/database.php';
  // Get the license number from the request data
  $licence = $_GET['licence'] ?? '';
  // Query the database with a prepared statement
  $req = getConnection()->prepare('SELECT * FROM joueur WHERE licence = :licence');
  $req->execute(['licence' => $licence]);
  $rows = $req->fetchAll();
    echo "<h1>Détails de la licence</h1>";
    echo "<body>";
    echo "<div>";
    foreach ($rows as $row) {
      // Print each element of the query result in a clean HTML page
      echo '<p> Licence : ';
      echo htmlspecialchars($row['licence']);
      echo '</p>';
      echo '<p>Nom : ';
      echo htmlspecialchars($row['nom']);
      echo '</p>';
      echo '<p>Prénom : ';
      echo htmlspecialchars($row['prenom']);
      echo '</p>';
      echo '<p>ID Equipe : ';
      echo htmlspecialchars($row['id_equipe']);
      echo '</p>';
      echo '<p>Poste : ';
      echo htmlspecialchars($row['poste']);
      echo '</p>';
    }
?>
    <button class='changePage' onclick="location.href='../decoder.html'">Décodeur ST-CODE</button>
    <?php
    echo "</div>";


    echo "</body>";
    
    print "<style>
      body{
        display:flex;
        flex-direction:column;
        align-items:center;
        background-color:black;
      }
      h1{
        color: aliceblue;
        font-size:60px;
        font-family : Arial, Helvetica, sans-serif;
        align-items: center;
      }
      div{
        display:flex;
        flex-direction:column;
        align-items:center;
        margin-top:100px;
        test-align:center;
      }
      div p{
      color:white;
      font-size: 16px;
      font-weight: bold;
    }
    button {
      margin-top:40px;
      display: inline-block;
      font-size: 16px;
      padding: 10px 20px;
      color: white;
      background-color: #f9b233;
      border: none;
      border-radius: 4px;
      cursor: pointer;
    }
      button:hover {
        background-color: #b78022;
    }
  </style>";

?>
